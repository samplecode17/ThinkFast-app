    ...
    <div class="pl-4 mt-4">
        <div class="bg-white p-4 rounded-lg ">
            <div class="flex justify-between items-center">
                <div class="flex justify-between items-center">
                    <img class="size-8 rounded-full" :src="comment?.user_image" alt="User avatar" />
            ...
            <div v-if="showReply" class="mt-3">
                <textarea v-model="replyText" placeholder="Write your reply..."
            ...
            <div v-if="comment.replies && comment.replies.length" class="mt-2 ml-2 text-sm">
                <button @click="showReplies = !showReplies" class="text-blue-600 hover:underline">
                    {{ showReplies ? 'Hide replies' : `View ${comment.replies.length} repl${comment.replies.length === 1
                    ? 'y' : 'ies'}` }}
                </button>
            </div>
            ...
            <div v-if="showReplies && comment.replies && comment.replies.length" class="mt-4 space-y-4">
                <CommentItem v-for="reply in comment.replies" :key="reply.id" :comment="reply" :game-id="gameId"
