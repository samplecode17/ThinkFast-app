<template>
    ...
    <span class="col-start-1 row-start-1 flex items-center gap-3 pr-6">
        <span class="block truncate">{{ selectedDifficulty?.name || 'Select a difficulty' }}</span>
    </span>
    ...
    >
        ...
        <!-- <img :src="difficulty.imageUrl" alt="" class="size-5 shrink-0 rounded-full"> -->
        <span class="ml-3 block truncate font-normal">{{ difficulty.name }}</span>
        </div>
        <span v-if="difficulty.id === selectedDifficulty?.id" class="absolute inset-y-0 right-0 flex items-center pr-4 text-thinkfast">
...
onBeforeMount(async () => {
await store.dispatch('getDifficulties')
})

const emit = defineEmits(['select'])

// Reactive access to difficulties from the store
const difficulties = computed(() => store.getters.stateDifficulties)

// Local component state
const isOpen = ref(false)
const selectedDifficulty = ref(null) // Currently selected difficulty object
